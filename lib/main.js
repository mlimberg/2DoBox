...
<<<<<<< HEAD
  tasksArray.forEach(function(task, index, tasksArray) {
    ...
    let tenthFromLast = tasksArray.length - 10;
    if(index <= lastItem && index >= tenthFromLast) {
      task.hidden = false;
    }
    displayOnPage(task);
=======
  let topTenArray = tasksArray.slice(-10);
  topTenArray.forEach((task) => {
    displayOnPage(task, '.task-list');
>>>>>>> 56084f71b2eea119bc07b1274607f77e79ac1fc7
  });
}
...
displayOnPage = currentTask => {
  ...
  if(currentTask.hidden === true) {
    currentTask.articleClass = ' hidden';
  } else {
    currentTask.articleClass = '';
  }
  $('.task-list').prepend(
    `<article id=${currentTask.uniqueid} class="task-box${currentTask.articleClass}">
    ...
    <button id="completed-task" class="btn">Completed Task</button>
    </div>
    </article>`);
  };
